...
def get_data_from_db():
    conn_str = (
        "mssql+pyodbc://sa:tQs73Z39XcGi@76.74.150.73/FabesBD?driver=ODBC+Driver+17+for+SQL+Server"
    )
    query = """
    IF OBJECT_ID('tempdb..#PrestamosActivos') IS NOT NULL
        DROP TABLE #PrestamosActivos;

    SELECT 
        FORMAT(pa.FechaRecuperacion, 'dd/MM/yyyy') AS Fecha,
        '202413' AS Quincena,
        p.IdPrestamo,
        m.RFC,
        m.Nombre,
        (pa.Capital + pa.Interes + ISNULL(pa.FondoGarantia, 0)) AS Descuento,
        pa.Capital AS PagoCapital,
        pa.Interes AS PagoInteres,
        pa.FondoGarantia AS PagoFondoGarantia,
        pa.IdTipoPago,
        tp.TipoPago
    INTO #PrestamosActivos
    FROM 
        Prestamos p
    JOIN 
        Maestros m ON p.IdMaestro = m.IdMaestro
    JOIN 
        Pagos pa ON p.IdPrestamo = pa.IdPrestamo
    JOIN 
        TipoPago tp ON pa.IdTipoPago = tp.IdTipoPago
    WHERE 
        pa.QnaProceso = '202413'
        AND pa.Activo = 0
        AND tp.TipoPago <> 'CANCELACION X FG'
        AND tp.TipoPago <> 'ABONO X BENEFICIO INVALIDEZ'
        AND tp.TipoPago <> 'APLICACION X FG'
        AND tp.TipoPago <> 'DEVOLUCION'
        AND tp.TipoPago <> 'CANCELACION DE DEVOLUCION BANCARIA'
    ORDER BY  
        p.FechaSol, p.IdPrestamo;
    ...
    try:
        ...
        messagebox.showerror("Error", f"Se produjo un error al obtener los datos de la base de datos: {e}")
        return pd.DataFrame()  # Retorna un DataFrame vacío en caso de error
def compare_excel_files(file1_path):
    try:
        # Leer el archivo Excel sin encabezados y especificando las columnas por índice.
        df1 = pd.read_excel(file1_path, usecols=[4, 6], header=None, names=['RFC', 'Descuento'], skiprows=1)
        
        # Obtener los datos de la base de datos.
        df2 = get_data_from_db()
        ...
        total_label_file2 = tk.Label(scrollable_frame, text=f"Total Descuento Base de Datos: {total_descuento_file2:.2f}", font=("Helvetica", 12))
        ...
        for index, row in missing_in_file2.iterrows():
            tree_missing.insert("", "end", values=(row['RFC'], "Faltante en Base de Datos"))
        ...
        tree_different = ttk.Treeview(scrollable_frame, columns=("RFC", "Descuento Archivo 1", "Descuento Base de Datos"), show='headings', selectmode="extended")
        tree_different.heading("RFC", text="RFC")
        tree_different.heading("Descuento Archivo 1", text="Descuento Archivo 1")
        tree_different.heading("Descuento Base de Datos", text="Descuento Base de Datos")
        ...
        messagebox.showerror("Error", "Las columnas requeridas no están presentes en el archivo.")
    except Exception as e:
...
def compare_files():
    if file1_path:
        compare_excel_files(file1_path)
    else:
        messagebox.showerror("Error", "Por favor, selecciona el archivo Excel.")
...
compare_button = tk.Button(root, text="Comparar archivo Excel con base de datos", command=compare_files)
